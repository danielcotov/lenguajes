...
BEGIN
    SELECT TO_DATE('01-NOV-' || EXTRACT (YEAR FROM ADD_MONTHS(TRUNC(SYSDATE), 12 * 1 * -1)))
    INTO VCORTE
    FROM DUAL;
 RETURN VCORTE;
END;
...
AS
 VMESES NUMBER(6);
BEGIN
    SELECT TRUNC(MONTHS_BETWEEN(CORTE_AGUINALDO,FECHA_INGRESO)) 
    INTO VMESES
    FROM USUARIOS
    WHERE ID_USUARIO = VID;
...
SELECT MESES_AGUINALDO(5) FROM DUAL;
