...
CREATE OR REPLACE PROCEDURE LISTAR_CLIENTE(VID IN NUMBER, NOMBRE OUT VARCHAR2,APELLIDO OUT VARCHAR2,DIRECCION OUT VARCHAR2,
                                            PAIS OUT VARCHAR2,PROVINCIA OUT VARCHAR2,
                                            CANTON OUT VARCHAR2,CORREO OUT VARCHAR2,
...
CREATE OR REPLACE PROCEDURE ACTUALIZAR_CLIENTE(VID IN NUMBER,VNOMBRE IN VARCHAR2,VAPELLIDO IN VARCHAR2,VDIRECCION IN VARCHAR2, 
                                            VCANTON IN VARCHAR2,VCORREO IN VARCHAR2,
                                            VTELEFONO IN NUMBER, VGENERO IN CHAR,
                                            VFECHA_NACIMIENTO IN DATE)
AS
    VCANT_C NUMBER(6) :=CANTIDAD_CANTONES + 1;
BEGIN              
    INSERT INTO CANTONES
    SELECT VCANT_C,VCANTON
    FROM DUAL 
    WHERE NOT EXISTS(SELECT NOMBRE FROM CANTONES
                     WHERE NOMBRE = VCANTON);  
